...
## 11. 2026-06-23 — full-capability expansion (v1.79; current, sourced)

Extensive research refresh + build-out of `services/relocation_tax.py` from the P1 core (5 rate corridors) to the full deterministic, decision-support engine the brief envisaged.

**Research (current, sourced):**
- **Italy flat tax:** EUR 100k → **EUR 200k** for new optants from the 2025 period (Aug-2024 decree; pre-Aug-2024 entrants grandfathered at EUR 100k), → **EUR 300k from 1 Jan 2026** for new residents (2026 Budget Law). PLUS a **7% flat on foreign pension/income** for retirees in <20,000-population Southern-Italy towns (Sicily/Calabria/Puglia/etc.), 10 yrs. [PwC Tax Summaries (Italy); Agenzia delle Entrate; Italy Budget Law 2026]
- **Switzerland lump-sum (forfait):** federal **minimum taxable base CHF 434,700** (2025), taxed at ordinary cantonal rates → indicative tax ~CHF 130k–200k; only for foreigners NOT gainfully employed in CH, new (or 10+ yrs absent); cantons vary (Ticino/Vaud favourable; Zurich abolished it). [KPMG CH; Swiss Federal Dept of Finance]
- **Greece:** EUR 100k flat on all foreign income (HNWI non-dom, 15 yrs, needs EUR 500k investment, not GR-resident 7/8 yrs) + a separate **7% foreign-pension** regime. [Astons; PwC; Grant Thornton GR]
- **SA CFC (s9D):** a >50%-SA-held foreign company's net income is attributed to SA-resident holders (>=10%) unless the FBE or high-tax (>= ~21%) exemption applies → relocating the person but keeping the company often doesn't achieve the saving. [National Treasury s9D; CDH; BDO 2025]
- **Investment-residency thresholds (2025):** Greece Golden Visa EUR 250k–800k (EUR 800k Athens/Thessaloniki/islands); Portugal Golden Visa EUR 500k funds/VC (real-estate route removed) or EUR 250k donation; UAE Golden Visa AED 2m property; Malta residence EUR 375k property (+~EUR 60k fees); Mauritius ~US$375k property; Cyprus PR ~EUR 300k; Italy investor visa EUR 250k/500k/2m. [Henley & Partners; Astons; Get Golden Visa 2025]
- **SA double-tax treaties:** SA has comprehensive DTAs with all 8 corridors (UAE/Mauritius/Cyprus/Malta/Portugal/Greece/Italy/Switzerland) — 79 treaties total; tie-breaker = TRC / permanent home / centre of vital interests / habitual abode. [SARS DTA list; PwC]

**Built (v1.79):** corridor count **5 → 8** — added **Greece, Italy, Switzerland** as a new **flat-fee regime type** (`regime: "flat_fee"`, with `flat_fee_zar/local/note`, converted at a dated indicative FX ~R20/EUR, ~R21/CHF). Quantification now handles flat-fee corridors: a FIXED fee on foreign income, so a flat-fee corridor is `strong` ONLY when current SA tax exceeds the fee (it correctly shows `weak`/negative-saving for modest earners — the honest "only for the very wealthy" result). Added a per-corridor enrichment layer (`investment_route`, `substance`, `dta_with_sa`) for all 8; a 6th **CFC guardrail** (model the company, not just the person); a 7-step **compliant sequencing** checklist (intent → exit-CGT → cease SA residency + RAV01 → establish residency/substance → company/CFC → CRS/DAC6 → licensed advisor); and a **cost_considerations** block + `fx_assumption`. Still pure-deterministic, no LLM in the numbers; decision-support, NOT advice; legal relocation/efficiency only — never evasion. +6 tests; 430 pass.

**Still gated / NOT built (deferred, by design):** (a) the **LLM narration layer** (a personalised, faithfulness-checked brief) — changes real model behaviour, so it needs a live A/B per the discipline; (b) the **licensed-advisor + execution marketplace** + **PI insurance** + **DAC6/MDR registration** + **demand validation** — the **P0**, which remains Ruan/lawyer-led and is the prerequisite before this is a real product; (c) a frontend planner page. The engine is the deterministic, professionally-supervisable first-pass only.
